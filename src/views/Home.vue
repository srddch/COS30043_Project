...
// --- Task Manager Logic (Clean Slate) ---
const newTask = ref('')
const tasks = ref([]) // 初始数据已清空
      ...
      <div class="col-md-5">
        <div class="p-3 bg-white shadow-sm rounded-3 border">
          ...
          <div class="progress" style="height: 8px;">
            <div class="progress-bar bg-success transition-all" :style="{ width: progress + '%' }"></div>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="col-lg-4">
        <div class="card shadow-sm border-0 h-100 bg-dark text-white">
          <div class="card-body text-center d-flex flex-column justify-content-center py-5">
            ...
            <div class="display-1 fw-bold mb-4 font-monospace">{{ formattedTime }}</div>
            <div class="d-grid gap-2 d-md-block">
              <button @click="toggleTimer" :class="['btn px-4 me-md-2', isRunning ? 'btn-outline-warning' : 'btn-primary']">
                {{ isRunning ? 'Pause' : 'Start' }}
              </button>
              <button @click="resetTimer" class="btn btn-outline-light px-4">Reset</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="card shadow-sm border-0 h-100">
          ...
          <div class="card-body pt-0">
            <div class="input-group mb-3">
              <input v-model="newTask" @keyup.enter="addTask" type="text" class="form-control border-primary-subtle" placeholder="What is your main goal today?">
              <button @click="addTask" class="btn btn-primary px-4">Add Task</button>
            </div>
            
            <div class="list-group list-group-flush overflow-auto" style="max-height: 200px;">
              <div v-for="task in tasks" :key="task.id" class="list-group-item d-flex justify-content-between align-items-center border-0 px-0">
                <div class="form-check">
                  <input v-model="task.done" class="form-check-input shadow-none" type="checkbox" :id="'t-'+task.id">
                  <label class="form-check-label ms-2" :class="{'text-decoration-line-through text-muted': task.done}" :for="'t-'+task.id">
                    {{ task.text }}
                  </label>
                </div>
                <button @click="tasks = tasks.filter(t => t.id !== task.id)" class="btn btn-sm btn-link text-danger text-decoration-none">Delete</button>
              </div>
              
              <!-- Empty State -->
              <div v-if="tasks.length === 0" class="text-center py-5 text-muted">
                <p class="mb-0 small italic">Your task list is empty. Add a task to start tracking progress!</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="col-md-4">
        <div class="card h-100 shadow-sm border-0 border-top border-primary border-4">
          <div class="card-body">
            ...
            <router-link to="/courses" class="btn btn-outline-primary w-100">Explore Courses</router-link>
          </div>
        </div>
      </div>
      ...
      <div class="col-md-4">
        <div class="card h-100 shadow-sm border-0 border-top border-success border-4">
          <div class="card-body">
            ...
            <router-link to="/forum" class="btn btn-outline-success w-100">Go to Forum</router-link>
          </div>
        </div>
      </div>
      ...
      <div class="col-md-4">
        <div class="card h-100 shadow-sm border-0 border-top border-warning border-4">
          <div class="card-body">
            ...
            <router-link to="/schedule" class="btn btn-outline-warning w-100">View Schedule</router-link>
          </div>
        </div>
      </div>
    </div>
...
<style scoped>
.transition-all { transition: width 0.4s ease-in-out; }
